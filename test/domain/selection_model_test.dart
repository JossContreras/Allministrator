...
void main() {
  test('selection group is immutable, ordered and chooses deterministic primary', () {
  ...
  test('controller supports add, toggle and visual ranges with one source of truth', () {
    ...
    expect((controller.context.currentSelection as MultiBlockSelection).group.blockIds, ['b', 'c', 'a']);
    controller.dispatch(const SelectRangeIntent('d', visualOrder: ['a', 'b', 'c', 'd']));
  ...
  test('resolver maps Ctrl/Cmd and Shift block activation to selection intents', () {
    ...
    NormalizedInputEvent input(InputModifiers modifiers) => NormalizedInputEvent(
    ...
    expect(
      resolver.resolve(event: input(const InputModifiers(control: true)), context: const InteractionContext()).intent,
      isA<ToggleBlockSelectionIntent>(),
    );
    expect(
      resolver.resolve(event: input(const InputModifiers(shift: true)), context: const InteractionContext()).intent,
  ...
  test('marquee bounds select only blocks with a meaningful intersection', () {
    ...
    registry.updateViewport(const WorkspaceViewportGeometry(globalBounds: SpatialRect.fromLTWH(0, 0, 400, 400)));
    ...
    expect(resolver.intersecting(const SpatialRect.fromLTWH(0, 20, 200, 130)), ['a', 'b']);
    expect(resolver.resolve(const SelectionGroup(blockIds: ['a', 'c'])), const SpatialRect.fromLTRB(0, 0, 200, 280));
  });

  test('marquee commits temporary candidates without persistence side effects', () {
    final controller = WorkspaceInteractionController();
    addTearDown(controller.dispose);
    controller.dispatch(const UpdatePointerIntent(InteractionPointer(pointerId: 1, position: InteractionPoint(0, 0), isDown: true)));
    controller.dispatch(const BeginMarqueeSelectionIntent(InteractionPoint(0, 0)));
    controller.dispatch(const UpdateMarqueeSelectionIntent(position: InteractionPoint(100, 100), candidateIds: ['a', 'b']));
    expect((controller.context.currentSelection as MultiBlockSelection).group.isTemporary, isTrue);
    controller.dispatch(const CommitMarqueeSelectionIntent());
    final committed = controller.context.currentSelection as MultiBlockSelection;
    expect(committed.group.blockIds, ['a', 'b']);
    expect(committed.group.isTemporary, isFalse);
  });

  test('Delete delegates to text editing and targets blocks outside editing', () {
